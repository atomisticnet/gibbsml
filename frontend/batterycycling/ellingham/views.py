...
def ellingham(request):
    ...
    # Compile the data (oxide list & blank forms) to be sent to the
    # webpage template.
    context = {
    ...
    if request.method == 'GET':
        ...
        if new_oxide is not None:
            ...
            if new_oxide == '':
                ...
                oxide_error = ("'{}'".format(new_oxide)
                               + " has already been added to the list!")
            elif (new_oxide[:3] == 'mp-'
                  and new_oxide.split('mp-', 1)[1].isdigit()):
                pass  # Allow entries with MP ID syntax to go through.
            else:
                try:
                    comp = Composition(new_oxide)
                    if comp.is_element:
                        oxide_error = "'{}' is an element!".format(new_oxide)
                except:
                    oxide_error = (
                            "'{}' is an invalid formula!".format(new_oxide))
    ...
    if request.method == 'POST':
        ...
        if filled_form.is_valid():
            ...
            # Strings for announcing successful calculations (with
            # unicode & without):
            select_string = ''
            plaintext_string = ''
            ...
            # Catch invalid metal oxides & API keys, and gather info for
            # the valid ones.
            with MPRester(USER_API_KEY) as mpr:
                for oxide in select_oxides.copy():
                    try:
                        plaintext_formulae.append(mpr.get_data(
                                oxide, prop='pretty_formula'
                                )[0]['pretty_formula'])
                    except IndexError:
                        ...
                        api_key_error = ("Invalid Materials Project "
                ...
                for idx, oxide in enumerate(select_oxides):
                    if oxide[:3] == 'mp-':
                        presentable_formulae.append(
                                unicodeify(plaintext_formulae[idx]))
                        table_ids.append(oxide)
                    else:
                        presentable_formulae.append(unicodeify(oxide))
                        table_ids.append(sorted(mpr.get_data(oxide),
                                                key=lambda k: k['e_above_hull']
                                                )[0]['material_id'])
            ...
            presentable_formulae[:] = [
                    x for x in presentable_formulae if x != '']
            ...
            def get_dG0_CO_expt(T):
                # Experimental free-energy of CO from the NIST-JANAF
                # database. Malcolm W. Chase, Jr. NIST-JANAF
                # Thermochemical Tables. Washington, DC : New York
                # :American Chemical Society ; American Institute of
                # Physics for the National Institute of Standards and
                # Technology, 1998.
                dG0_CO_expt = -2.23623129e+02 - 1.75809004e-01 * T
                return dG0_CO_expt

            # Temperature range, from 0K to 3000K.
            T_range = np.arange(0, 3001, 1)
            dG0_CO = []
            for i in T_range:
            ...
            for idx, id_mo in enumerate(select_oxides.copy()):
                try:
                    ...
                    try:
                        plain_rxn = BalancedReaction.from_string(
                                ellingham.get_balanced_reaction()
                                ).normalized_repr_and_factor()[0]
                    ...
                    output.append([presentable_formulae[idx],
                                   plaintext_formulae[idx], table_ids[idx],
                                   round(pred_dH0, 3), round(pred_dS0, 3),
                                   round(pred_dG0, 3), rxn, plain_rxn])

                    # Add data for each metal oxide calculation to the
                    # output diagram.
                    dG0_MO = []
                    for i in T_range:
            ...
            # Construct the error string for failed oxide calculations
            # (handled for multiple).
            fail_length = len(oxide_failures)
            if fail_length > 2:
                for failure in oxide_failures[:-1]:
                    calc_error += "'%s', " % (failure)
                calc_error += ("& '{}' ".format(oxide_failures[-1])
                               + "are all invalid metal oxides and have "
                               + "been removed from your list. ")
            elif fail_length == 2:
                calc_error += ("'{}' & '{}' ".format(oxide_failures[0],
                                                     oxide_failures[1])
                               + "are both invalid metal oxides and have "
                               + "been removed from your list. ")
            elif fail_length == 1:
            ...
            # Add the error string to the data for the webpage if there
            ...
            # Make the titles for the output diagram, table, &
            # downloadable table.
            ...
            headers = [oxide_header, id_header, formation_header,
                       entropy_header, free_header, rxn_header]
